...
bool CListUI::HotItem(int iIndex, bool bHot)
{
	return true;
}
bool CListUI::ClickItem(int iIndex, bool bTakeFocus)
{
	return true;
}
bool CListUI::DBClickItem(int iIndex, bool bTakeFocus)
{
	return true;
}
...
void CListUI::SetAlternateBkImage(LPCTSTR pStrImage)
{
    m_ListInfo.sAlternateBkImage = pStrImage;
    Invalidate();
}
...
LPCTSTR CListUI::GetItemAlternateBkImage() const
{
    return m_ListInfo.sAlternateBkImage;
}
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if( _tcscmp(pstrName, _T("itemalternatebkimage")) == 0 ) SetAlternateBkImage(pstrValue);
...
LPVOID CListHeaderItemUI::GetInterface(LPCTSTR pstrName)
{
    ...
    else if( _tcscmp(pstrName, DUI_CTR_LISTHEADERITEM) == 0 ) return this;
...
void CListHeaderItemUI::SetPos(RECT rc)
	...
	/*void CListHeaderItemUI::DoPostPaint(HDC hDC, const RECT& rcPaint)
	{
		if( (m_uButtonState & UISTATE_CAPTURED) != 0) {
			RECT rcSeparator = GetThumbRect();
...
bool CListElementUI::Select(bool bSelect,bool bCallback)
{
    ...
    m_bSelected = bSelect;
    if(bCallback && bSelect && m_pOwner != NULL ) m_pOwner->SelectItem(m_iIndex);
    Invalidate();
...
bool CListLabelElementUI::IsHoted() const
{
	return (m_uButtonState & UISTATE_HOT)?true:false;
}
bool CListLabelElementUI::Hot(bool bHot)
{
    if( !IsEnabled() ) return false;
	if( bHot)
	{
...
bool CListLabelElementUI::Click()
{
    if( !IsEnabled() ) return false;
...
bool CListContainerElementUI::IsHoted() const
{
	return (m_uButtonState & UISTATE_HOT)?true:false;
}
bool CListContainerElementUI::Hot(bool bHot)
{
    if( !IsEnabled() ) return false;
	if( bHot)
	{
...
bool CListContainerElementUI::Click()
{
    if( !IsEnabled() ) return false;
...
//@xdrt81y@20140125@add
bool CListContainerElementUI::DBClick()
{
    if( !IsEnabled() ) return false;
	
	m_pOwner->DBClickItem(m_iIndex);
	return true;
}
