...
bool DuiLib::CWebBrowserUI::DoCreateControl()
{
	...
	GetControl(IID_IHTMLWindow2,(LPVOID*)&pHTMLWindow);

	if(m_pWebBrowser2->Navigate(L"about:blank",0,0,0,0) == S_OK )
	{
	}
	return true;
}
...
STDMETHODIMP DuiLib::CWebBrowserUI::QueryInterface( REFIID riid, LPVOID *ppvObject )
{
	*ppvObject = NULL;
	...
	if(CActiveXUI::GetControl(riid, ppvObject) == S_OK)
	{
		return S_OK;
	}
...
void DuiLib::CWebBrowserUI::NavigateComplete2( IDispatch *pDisp,VARIANT *&url )
{
	...
	//GetModuleFileName(NULL,lpszModule,MAX_PATH);
	wstring wsurl = L"res://DuiLib2.dll/23/101";

	if(lstrcmp(url->bstrVal,L"about:blank") == 0)
	{
		CComPtr<IDispatch> spDoc = NULL;
		while(spDoc == NULL)
		{
		...
		m_pWebBrowser2->Navigate((BSTR)wsurl.c_str(),0,0,0,0);
		return;
	}

	if(lstrcmp(url->bstrVal,wsurl.c_str()) == 0)
	{
...
IDispatch* DuiLib::CWebBrowserUI::GetHtmlWindow()
{
	...
	hr = GetControl(IID_IWebBrowser2, (void**)&pWebBrowser2);
	if (FAILED(hr))
		return NULL;

	IDispatch* pDp =  NULL;
	hr=pWebBrowser2->get_Document(&pDp);
	if (FAILED(hr))
		return NULL;
	...
	hr=pHtmlWnd2->QueryInterface(IID_IDispatch, (void**)&pHtmlWindown);
	if (FAILED(hr))
		return NULL;
