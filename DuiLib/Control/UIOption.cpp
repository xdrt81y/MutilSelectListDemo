...
namespace DuiLib
{
	COptionUI::COptionUI() : m_bSelected(false), m_dwSelectedTextColor(0), m_dwSelectedHotTextColor(0)
	{
	}
	...
	void COptionUI::SetSelectedHotTextColor(DWORD dwTextColor)
	{
		m_dwSelectedHotTextColor = dwTextColor;
	}

	DWORD COptionUI::GetSelectedHotTextColor()
	{
		if (m_dwSelectedHotTextColor == 0) m_dwSelectedHotTextColor = m_pManager->GetDefaultFontColor();
		return m_dwSelectedHotTextColor;
	}
	//************************************
	...
	LPCTSTR COptionUI::GetSelectedHotForeImage()
	{
		return m_sSelectedHotForeImage;
	}

	void COptionUI::SetSelectedHotForeImage(LPCTSTR pStrImage)
	{
		m_sSelectedHotForeImage = pStrImage;
		Invalidate();
	}
	...
	void COptionUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("selectedhotforeimage")) == 0 ) SetSelectedHotForeImage(pstrValue);
		...
		else if( _tcscmp(pstrName, _T("selectedhottextcolor")) == 0 ) {
			if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
			SetSelectedHotTextColor(clrColor);
		}
	...
	void COptionUI::PaintStatusImage(HDC hDC)
	{
		...
		if((m_uButtonState & UISTATE_SELECTED) != 0)
		{
			if((m_uButtonState & UISTATE_HOT) != 0)
			{
				...
				if( !m_sSelectedHotForeImage.IsEmpty() ) {
					if( !DrawImage(hDC, (LPCTSTR)m_sSelectedHotForeImage) )
						m_sSelectedHotForeImage.Empty();
				} else if( !m_sSelectedForeImage.IsEmpty() ) {
	...
	void COptionUI::PaintText(HDC hDC)
	{
		if( (m_uButtonState & UISTATE_SELECTED) != 0 )
		{
			...
			if((m_uButtonState & UISTATE_HOT) != 0)
			{
				if( m_dwSelectedHotTextColor != 0 )
				{
					m_dwTextColor = m_dwSelectedHotTextColor;
				} else if(m_dwSelectedTextColor != 0)
				{
